...
function Item({
  ...
  deleteItem,
  auth,
  item: { _id, description, name, picture, price, species, likes, comments, date },
  showActions
}) {
  ...
  return (
    <Card className='root'>
      ...
      <CardActions disableSpacing className={classes.space}>
        ...
        <Link to={`/items/${_id}`} className='btn btn-primary'>
            Discussion{' '}
            {comments.length > 0 && (
